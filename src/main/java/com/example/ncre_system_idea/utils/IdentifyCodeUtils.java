...
public class IdentifyCodeUtils {
    ...
    private Font getFont() {
        return new Font("Times New Roman", Font.CENTER_BASELINE, 40);
    }
    ...
    public BufferedImage getIdentifyImage(String identifyCode) {
        ...
        //字体大小
        graphics.setFont(new Font("Times New Roman", Font.ROMAN_BASELINE, 60));/*被getFont()方法覆盖了，这行最后不会影响到图片的字体*/
        //字体颜色
    ...
    private void drawString(Graphics g, String identifyCode) {
        int charWidth = width / identifyCode.length();
        int charHeight = height - 10; // 调整字符的垂直位置

        for (int i = 0; i < identifyCode.length(); i++) {
            g.setFont(getFont());    //字体大小
            g.setColor(new Color(random.nextInt(101), random.nextInt(111), random.nextInt(121)));//字体颜色

            int x = charWidth * i + charWidth / 2 - 10; // 计算字符的水平位置
            int y = charHeight;

            g.translate(random.nextInt(3), random.nextInt(3));
            g.drawString(String.valueOf(identifyCode.charAt(i)), x, y);
        }
